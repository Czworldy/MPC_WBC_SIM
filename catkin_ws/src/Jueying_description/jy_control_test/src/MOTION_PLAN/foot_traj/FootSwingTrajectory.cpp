...
void FootSwingTrajectory<T>::computeSwingTrajectoryBezier(T phase, T swingTime) {
  _p = Interpolate::cubicBezier<Vec31<T>>(_p0, _pf, phase);
  _v = Interpolate::cubicBezierFirstDerivative<Vec31<T>>(_p0, _pf, phase) / swingTime;
  _a = Interpolate::cubicBezierSecondDerivative<Vec31<T>>(_p0, _pf, phase) / (swingTime * swingTime);
