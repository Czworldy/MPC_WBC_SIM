#include "pronto_jypro_commons/forward_kinematics.hpp"
...
Vector3d ForwardKinematics::getFootPosLF(const JointState& q){
    return Utils::positionVector(ht_.fr_BASE_X_fr_LF_FOOT(q));
}

Vector3d ForwardKinematics::getFootPosRF(const JointState& q){
    return Utils::positionVector(ht_.fr_BASE_X_fr_RF_FOOT(q));
}

Vector3d ForwardKinematics::getFootPosLH(const JointState& q){
    return Utils::positionVector(ht_.fr_BASE_X_fr_LH_FOOT(q));
}

Vector3d ForwardKinematics::getFootPosRH(const JointState& q){
    return Utils::positionVector(ht_.fr_BASE_X_fr_RH_FOOT(q));
}
...
Matrix3d ForwardKinematics::getFootOrientation(const JointState &q, const LegID &leg){
    ...
    switch(leg){
    case LegID::LF:
        return Utils::rotationMx(ht_.fr_BASE_X_fr_LF_FOOT);
    case LegID::RF:
        return Utils::rotationMx(ht_.fr_BASE_X_fr_RF_FOOT);
    case LegID::LH:
        return Utils::rotationMx(ht_.fr_BASE_X_fr_LH_FOOT);
    case LegID::RH:
        return Utils::rotationMx(ht_.fr_BASE_X_fr_RH_FOOT);
    default:
