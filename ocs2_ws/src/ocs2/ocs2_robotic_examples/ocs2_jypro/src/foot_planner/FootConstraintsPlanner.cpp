    ...
    if(1){
      ...
      for (int p = 0; p < modeSequence.size(); ++p) {
        ...
        if (!eesContactFlagStocks[j][p]) { // for all swing phases 
          ...
          matrix_t A, conA; vector_t b, zeroCol;
          polygon.convertToInequalityConstraints(A, b);
          zeroCol.resizeLike(b); zeroCol.setZero();
...
vector3_t FootConstraintsPlanner::choiceCloestFootPlacement(const size_t& footNum, const vector3_t& position){
