...
#define numOfArmJoint 6
#define numOfContactPoint 4
...
void grf_LF_callback(const gazebo_msgs::ContactsState::ConstPtr& msg);
void grf_RF_callback(const gazebo_msgs::ContactsState::ConstPtr& msg);
void grf_LH_callback(const gazebo_msgs::ContactsState::ConstPtr& msg);
void grf_RH_callback(const gazebo_msgs::ContactsState::ConstPtr& msg);
...
{   
    ...
    mpc_input = nh.advertise<ocs2_msgs::mpc_observation>("/legged_robot_with_arm_mpc_observation", 1);
    grf_LF = nh.subscribe("/LF_contact", 1, &grf_LF_callback);
    grf_RF = nh.subscribe("/RF_contact", 1, &grf_RF_callback);
    grf_LH = nh.subscribe("/LH_contact", 1, &grf_LH_callback);
    grf_RH = nh.subscribe("/RH_contact", 1, &grf_RH_callback);
    ...
    while(nh.ok()){
        if(jointStateMsg && grfLFMsg && grfRFMsg && grfLHMsg && grfRHMsg && gazeboStates_Msg){
            ...
            // mpc_input_msg
            mpc_input_msg.input.value.resize(3 * numOfContactPoint + 3 + numOfActuatedJoint);
            // Contact Force 
            uint index = 0;
            for(uint i = 0; i < numOfContactPoint; i++){
                if(mpcInputData.stance_bool_[i]){
                    ...
                    for(uint j = 0; j < 3; j++){
                        mpc_input_msg.input.value[index + j] = 0.0;
                    }
                    index += 3;
                }
            }
            // arm force 
            for (int i = 12; i < 15; i++){
                mpc_input_msg.input.value[i] = 0.0;
            }
            
            // Actuated joint
            for(uint i = 0; i < numOfActuatedJoint; i++){
                mpc_input_msg.input.value[3 * numOfContactPoint + 3 + i] = mpcInputData.v_[6 + i];
            }
            ...
            if(debug){
                ...
                std::cout << "Actuated Joints Position :" << std::endl;
                for(uint i = 0; i < numOfActuatedJoint; i++){
...
void jointStateCallback(const sensor_msgs::JointState::ConstPtr& msg){
    ...
    // arm 
    for (int i = 0; i < numOfArmJoint; i++){
        mpcInputData.q_[i+18] = msg->position[i+12];
        mpcInputData.v_[i+18] = msg->position[i+12];
    }
...
void grf_LF_callback(const gazebo_msgs::ContactsState::ConstPtr& msg){
    mpcInputData.stance_[0] = int(!msg->states.empty());
    mpcInputData.stance_bool_[0] = int(!msg->states.empty());

    // // LF
    // mpcInputData.stance_[0] = msg->lf; 
    // mpcInputData.stance_bool_[0] = msg->lf;
    // // RF
    ...
    if(mpcInputData.stance_bool_[0]){
        ...
        mpcInputData.grf_[0] = 0;
        mpcInputData.grf_[1] = 0;
        mpcInputData.grf_[2] = 0;
    }
...
void grf_RF_callback(const gazebo_msgs::ContactsState::ConstPtr& msg){
    mpcInputData.stance_[1] = int(!msg->states.empty());
    mpcInputData.stance_bool_[1] = int(!msg->states.empty());
    if(mpcInputData.stance_bool_[1]){
        Eigen::Matrix<double,3,1> grf, grf_tpl;
        grf[0] = msg->states[0].wrenches[0].force.x;
        grf[1] = msg->states[0].wrenches[0].force.y;
        grf[2] = msg->states[0].wrenches[0].force.z;
        ...
        mpcInputData.grf_[3] = 0;
        mpcInputData.grf_[4] = 0;
        mpcInputData.grf_[5] = 0;
    }
...
void grf_LH_callback(const gazebo_msgs::ContactsState::ConstPtr& msg){
    mpcInputData.stance_[2] = int(!msg->states.empty());
    mpcInputData.stance_bool_[2] = int(!msg->states.empty());
    if(mpcInputData.stance_bool_[2]){
        Eigen::Matrix<double,3,1> grf, grf_tpl;
        grf[0] = msg->states[0].wrenches[0].force.x;
        grf[1] = msg->states[0].wrenches[0].force.y;
        grf[2] = msg->states[0].wrenches[0].force.z;
        ...
        mpcInputData.grf_[6] = 0;
        mpcInputData.grf_[7] = 0;
        mpcInputData.grf_[8] = 0;
    }
...
void grf_RH_callback(const gazebo_msgs::ContactsState::ConstPtr& msg){
    mpcInputData.stance_[3] = int(!msg->states.empty());
    mpcInputData.stance_bool_[3] = int(!msg->states.empty());
    if(mpcInputData.stance_bool_[3]){
        Eigen::Matrix<double,3,1> grf, grf_tpl;
        grf[0] = msg->states[0].wrenches[0].force.x;
        grf[1] = msg->states[0].wrenches[0].force.y;
        grf[2] = msg->states[0].wrenches[0].force.z;
        ...
        mpcInputData.grf_[9] = 0;
        mpcInputData.grf_[10] = 0;
        mpcInputData.grf_[11] = 0;
    }
    grfRHMsg = true;
}
