...
struct conversionData{
    ...
    vector_array_t armPosition;
    vector_array_t armVeclocity;
    vector_array_t armAcceleration;
};
...
legged_robot::ModelSettings modelSettings;
	...
	#define dimOfContactForce 12
	#define dofOfRobot 24
	#define numOfArmDof 6
#else
	#define numOfActuatedJoint 12 
	#define dimOfContactForce 12
	#define dofOfRobot 18
#endif
...
void KinematicDynamicSetup(const std::string& urdfTree);
...
int main(int argc, char **argv)
{
    ...
    mpcPolicySubscriber = nh.subscribe("/legged_robot_with_arm_mpc_policy", 3, &mpcPolicyCallback);
...
void mpcPolicyCallback(const ocs2_msgs::mpc_flattened_controller::ConstPtr& msg) {
    ...
    // init arm info
    wbcInterfaceData.armPosition.clear();
    wbcInterfaceData.armPosition.resize(N_times);
    wbcInterfaceData.armVeclocity.clear();
    wbcInterfaceData.armVeclocity.resize(N_times);
    wbcInterfaceData.armAcceleration.clear();
    wbcInterfaceData.armAcceleration.resize(N_times);
    ...
    for(size_t k = 0; k < N_times; k++){
        ...
        /// TODO: assume no contact force of arm 
        for (size_t j = 12; j < 15; j++){
            mpcData.inputTrajectory_[k][j] = 0.0;
        }
    ...
    for (int i = 0; i < N_times; i++){  
        ...
        //std::cout << "1" << std::endl;  
        for (int k = 0; k < modelSettings.contactNames3DoF.size()-1; k++){ // LF RF LH RH
        ...
        //std::cout << "2" << std::endl;  
        wbcMsg.wbcTraj[i].actJointPos.resize(numOfActuatedJoint);
        wbcMsg.wbcTraj[i].actJointVel.resize(numOfActuatedJoint);
        for (int j(0); j < numOfActuatedJoint; j++) {
          wbcMsg.wbcTraj[i].actJointPos[j] = q[i][6 + j];
          wbcMsg.wbcTraj[i].actJointVel[j] = v[i][6 + j];
        }
        wbcMsg.wbcTraj[i].armPos.resize(numOfArmDof);
        wbcMsg.wbcTraj[i].armVel.resize(numOfArmDof);
        wbcMsg.wbcTraj[i].armAcc.resize(numOfArmDof);
        for (int j = 0; j < numOfArmDof; j++){
          wbcMsg.wbcTraj[i].armPos[j] = q[i][18 + j];
          wbcMsg.wbcTraj[i].armVel[j] = v[i][18 + j];
          wbcMsg.wbcTraj[i].armAcc[j] = a[i][18 + j];
        }

        wbcMsg.wbcTraj[i].swingJointPos.resize(12);
        wbcMsg.wbcTraj[i].swingJointVel.resize(12);
        wbcMsg.wbcTraj[i].swingJointAcc.resize(12);
        for (int j = 0; j < 12; j++) {
          wbcMsg.wbcTraj[i].swingJointPos[j] = q[i][6 + j];
          wbcMsg.wbcTraj[i].swingJointVel[j] = v[i][6 + j];
          wbcMsg.wbcTraj[i].swingJointAcc[j] = a[i][6 + j];
        }
...
void getGeneralizedVelocities(){
    ...
    for(size_t k = 0; k < N_times; k++){
        ...
        // Actuated joints
        for(size_t j = 0; j < numOfActuatedJoint; j++){
            v[k][j + 6] = mpcData.inputTrajectory_[k][j + dimOfContactForce + 3];
        }
        // Base
    ...
    for(size_t k = 0; k < N_times; k++){
        ...
        for(size_t j = 0; j < N_contactPoint; j++){
            ...
            if(j==4){
              ...
              // wbcInterfaceData.armPosition[k] = data.oMf[model.getBodyId(modelSettings.contactNames3DoF[j])].translation();
              // wbcInterfaceData.armVeclocity[k] = (jacobianContactPoint * v[k]).head(4);
              // wbcInterfaceData.armAcceleration[k] = (jacobianContactPoint * a[k] + jacobianDotContactPoint * v[k]).head(4);
            }else{
        ...
        // wbcInterfaceData.armVeclocity[k].tail(1) = v[k].tail(1); 
        // wbcInterfaceData.armAcceleration[k].tail(1) = a[k].tail(1); 

    }
}

Eigen::Matrix<double, 4, 1> modeNumber2StanceLeg_WBC(const size_t& modeNumber){
    Eigen::Matrix<double, 4, 1> stanceLegs;
    switch (modeNumber)
    {
        case 0:
