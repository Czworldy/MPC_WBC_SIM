    ...
    //   armEeKinematics_(armEeKinematics.clone())
{
    ...
    // dynamic_reconfigure::Server<ocs2_wbc::WbcWeightConfig>::CallbackType cb = [this](auto&& PH1, auto&& PH2) {
    //     dynamicCallback(std::forward<decltype(PH1)>(PH1), std::forward<decltype(PH2)>(PH2));
    // };
    // dynamic_srv_->setCallback(cb);
}

void WbcBase::modifyWbcParameters() {
    frictionCoeff_ = userParam_.mu;
...
// Tracking base XYZ motion task
Task WbcBase::formulateBaseXYZMotionTask() {
    matrix_t a(3, numDecisionVars_);
    vector_t b(a.rows());

    a.setZero();
    b.setZero();
    a.leftCols(info_.generalizedCoordinatesNum) = base_j_.topRows(3);
    ...
    b = baseAccDesired_.head(3) + Kp * (qDesired_.head(3) - qMeasured_.head(3))
           + Kd * (vDesired_.head(3) - vMeasured_.head(3));
...
Task WbcBase::formulateTorqueLimitsTask() {
    ...
    f.head(info_.actuatedDofNum) -= hj;
    f.tail(info_.actuatedDofNum) += hj;
