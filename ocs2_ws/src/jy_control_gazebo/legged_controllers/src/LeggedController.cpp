...
void LeggedController::updateStateEstimation(const ros::Time &time, const ros::Duration &period) {
    ...
    qMeasured_.tail(leggedInterface_->getCentroidalModelInfo().actuatedDofNum) = measuredRbdState_.segment(6, leggedInterface_->getCentroidalModelInfo().actuatedDofNum);
...
void LeggedController::setupMpc() {
  ...
  mpc_ = std::make_unique<ocs2::MultipleShootingMpc>(leggedInterface_->mpcSettings(), leggedInterface_->sqpSettings(),
    ...
    auto polygonReceiverPtr = std::make_shared<ocs2::legged_robot::LegEndEffectorsPolygonReceiver>(nodeHandle, leggedInterface_->getSwitchedModelReferenceManagerPtr()->getMpcPolygonArrayPtr(),
